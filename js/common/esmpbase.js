$.ajaxSetup({
    ...
    complete:function(XMLHttpRequest,textStatus){
        ...
        // console.log("responseText:"+responseText+",path:"+window.location.pathname);
        if(responseText!=undefined&&responseText!=null&&responseText.indexOf("E00003")>-1){//token 失效，重新登录
    ...
    error: function (xhr, status, e) {
        xhr = null;
        if(typeof(console)!="undefined"){console.log('request  is failed:'+e)}
        //backToLogin();
    }
});
...
function backToLogin(){
    ...
    $.ajax({
        url : ctx + '/index_logout.action'
        ,success : function(){
            var url;
            if (sign == 'cloud') {
                url = operateSysUrl;
            } else if (sign == 'screen') {
                url =  getRootPath() +'/login.jsp';
            } else {
                url =  getRootPath() +'/login.jsp';
            }
